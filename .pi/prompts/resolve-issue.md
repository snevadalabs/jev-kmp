...
**Nothing to decide.** These tickets sit downstream of a locked brief. Where a ticket says nothing is to be decided and you find you need a decision that the brief, an ADR, or the ticket's own links do not already make, **stop and report the question**. Do not invent the decision. That is the one thing you may not unblock yourself, and a section in the brief that looks wrong is a finding to report, not a licence to overrule it. **When you stop this way, leave your worktree in place** — do not run the removal step at the end, so the parent can read the branch and your panel stays up. Everything else about the stop is the same: claim commit pushed, ticket left `claimed`, the question with its evidence in your reply.
