...
Amended: **§4** (escape-hatch ordering), **§5** (per-call retry is in; retries log one line per attempt), **§10** (rewritten), **§11** (one internal parser shared with `RateLimitError.retryAfterMs`), **§13** (the retry header is written by `modifyRequest`). Ticket 08's ADR 0004 is rewritten in place — no ADR exists on disk, so nothing is superseded. Ticket 10's retry section is rewritten: the tests port to Ktor's `delay { }` seam, and the HTTP-date conformance fixture through `MockEngine` becomes the regression test that pins `respectRetryAfterHeader = false`.
